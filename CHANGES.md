# dsh-veryskill（VerySkill）开发日志

## 2026-09-01 — 适配 DSH v0.1.2-alpha.3
...
- 版本号：`0829-0.1.1-rc.2` → `0901-0.1.2-rc.2`
- 清理源码目录中的 .bak 备份文件
...
驼峰命名 VerySkill + 图标 + 版本 pill + [ideasir][卸载][已最新][智能检测] 4 标签。

### 架构
- 服务端 `src/index.ts`：settings namespace `veryskill`（enabled 开关）
...
- 客户端 `src/client/index.tsx`：looklook 风格完整卡片
  - key = `veryskill`（匹配服务端 namespace）
  - 头部：图标 + VerySkill + 版本 pill + [ideasir][卸载][已最新/有更新][智能检测] + 箭头
...
5. **namespace 迁移**：skill-manager → veryskill，settings.yaml 正则替换即可
...
- 排除自身（dsh-veryskill），避免自禁用后无法恢复

**客户端 src/client/index.tsx：**
- VerySkill 卡片：只保留开关（+图标/版本/4标签/智能检测）
...
5. **设置存技能目录**：`~/.dsh/skills/<技能名>/.veryskill.json`，随技能走。
...
### 问题
VerySkill 卡片用的是内联样式 + 硬编码颜色（rgb(53,54,56)、rgba(255,255,255,0.12)），
